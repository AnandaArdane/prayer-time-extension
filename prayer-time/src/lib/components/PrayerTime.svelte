<script lang="ts">
    ...
    let { prayerTimes, currentTime }: Props = $props();
...
<section class="w-full px-4 lg:px-12 mb-4">
    <div class="bg-white p-4 lg:p-5 rounded-[1.5rem] lg:rounded-[2rem] shadow-sm border border-gray-100">
                ...
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex justify-between items-center px-1">
                        <div class="flex flex-col">
                            <span class="text-[7px] lg:text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Waktu Sekarang</span>
                            <h3 class="text-xs lg:text-sm font-black text-gray-700 tracking-wider">{timeString}</h3>
                        </div>
            ...
            <div class="w-full lg:w-2/3">
                <div class="grid grid-cols-5 gap-2 lg:gap-3">
                    {#each prayerTimes as prayer, i}
                        ...
                        >
                            <span class="text-[7px] lg:text-[9px] font-bold text-gray-400 uppercase mb-1">{prayer.name}</span>
                            <span class="text-xs lg:text-xl font-black text-theme-primary-600">{prayer.time}</span>
                            
                            {#if active && isExactTime}
                                <div class="absolute top-0 right-0 bg-green-500 text-white text-[6px] lg:text-[8px] font-bold px-2 py-0.5 rounded-bl-lg uppercase">
                                    SEKARANG
                                </div>
                            {/if}
                        </div>
